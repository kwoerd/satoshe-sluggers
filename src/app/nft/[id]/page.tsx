...
import { RarityDistributionSection } from "@/components/rarity-distribution-section";
...
export default function NFTDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch('/docs/complete_metadata.json')
      .then(res => res.json())
      .then((data) => {
        setCompleteMetadata(data);
        
        // Find the specific NFT data
        ...
        if (found) {
          ...
          setMetadata(found);
          setImageUrl(found.merged_data?.media_url || "/media/nfts/placeholder-nft.webp");
        } else {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground flex flex-col">
      <Navigation activePage="nfts" />
      <div className="max-w-7xl mx-auto py-4 sm:py-6 flex-grow pt-24 sm:pt-28 pb-16 sm:pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 xl:gap-12">
          ...
          <div className="space-y-6 order-1 lg:order-1">
            ...
            {/* Attributes Section - Left Column Only */}
            <div className="bg-neutral-800 rounded border border-neutral-700 p-4">
              <h3 className="text-lg font-semibold text-off-white mb-2">Attributes</h3>
              <p className="text-sm text-neutral-400 mb-4">Each value shows how many NFTs in this collection share the same trait.</p>
              <div className="grid grid-cols-2 gap-3">
                {attributes.map((attr: any, index: number) => {
                  const color = getColorForAttribute(attr.name);
                  const occurrence = attr.occurrence;
                  return (
                    <div key={index} className="bg-neutral-900 rounded p-3 border border-neutral-700">
                      <div className="flex justify-between items-start">
                        {/* Left side - Color and category name */}
                        ...
                        {/* Right side - Category value and percentage/proportion */}
                        <div className="text-xs text-neutral-400 text-right">
                          <p className="text-off-white">{attr.value}</p>
                          <p>{attr.percentage}% | {occurrence || 0} of 7,777</p>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div className="space-y-6 -mt-2 order-2 lg:order-2">
            ...
            {/* Rarity Distribution Chart - Right Column */}
            <div className="bg-neutral-800 rounded border border-neutral-700 p-4">
              <h3 className="text-lg font-semibold text-off-white mb-4">Rarity Distribution</h3>
