...
function FilterCategory({ title, color, options, twoColumns = false, icon, selected = [], onChange, traitCounts = {} }: FilterCategoryProps & { selected: string[], onChange: (selected: string[]) => void, traitCounts?: Record<string, Record<string, number>> }) {
  const [isOpen, setIsOpen] = useState(false) // FIXED: Start with categories closed by default
  ...
  return (
    <div className={`${isOpen ? 'pt-3 pb-3' : 'pt-1'}`}>
      <button
        ...
        className={`w-full flex items-center justify-between py-2 focus:outline-none ${isOpen ? `border-b ${borderColorClasses[color]} pb-2` : ''}`}
        style={{ color: "#fffbeb" }}
  ...
  return (
    <div className={`${isOpen ? 'pt-3 pb-3' : 'pt-1'}`}>
      <button
        ...
        className={`w-full flex items-center justify-between py-2 focus:outline-none ${isOpen ? `border-b border-orange-500 pb-2` : ''}`}
        style={{ color: "#fffbeb" }}
        aria-expanded={isOpen}
      >
        <div className="flex items-center gap-2">
...
export default function NFTSidebar({ searchTerm, setSearchTerm, searchMode, setSearchMode, selectedFilters, setSelectedFilters, traitCounts = {} }: NFTSidebarProps) {
  // Debug logging to see what traitCounts contains
  React.useEffect(() => {
    console.log('NFTSidebar traitCounts:', traitCounts);
    console.log('Available categories:', Object.keys(traitCounts));
  }, [traitCounts]);
  
  // Debounced search tracking
  const searchTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  
  const debouncedTrackSearch = useCallback((searchTerm: string) => {
  ...
  React.useEffect(() => {
    debouncedTrackSearch(searchTerm)
    ...
    >
      ...
      <FilterContext.Provider value={filterContextValue}>
        ...
        <FilterCategory
          ...
          options={traitCounts["eyewear"] ? Object.keys(traitCounts["eyewear"]).sort().map(value => ({ value, display: value })) : FALLBACK_OPTIONS.eyewear.map(value => ({ value, display: value }))}
